...
//Color hex copy for swatches
var clr = document.querySelectorAll('div.swatch-colour');
// Copy to clipboard functionality
var clipcolor = new Clipboard(clr);

clipcolor.on('success', function(e) {
		console.log(e);
		var text_color = window.getComputedStyle(e.trigger).color;
		var hex_triplet = e.text;
		$('#copy_message').html('Copied <strong>' + hex_triplet + '</strong>');
		$('#copy_message').css({
			'background-color': hex_triplet,
			'color': text_color,
			'display': 'block'
		});
		$('#copy_message').delay(2500).fadeOut(500, "linear");
});

clipcolor.on('error', function(e) {
		console.log(e);
});